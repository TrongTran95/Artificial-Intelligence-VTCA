...
# filename structure
path = '/Users/imac_vtca/Downloads/faces/' # path to the database
...
def reconstruct_images(pca, prefix_sub, prefix='', path='', limit=2):
    ...
    for person_id in range(1, limit+1):
        for state in ['centerlight']:
            ...
            # im: x, phan sau: x0, x: x_bar
            x = im.reshape(D, 1) - pca.mean_.reshape(D, 1)
            # encode
            z = U.T.dot(x)
            print("{} -- {} ++ {}".format(z, person_id, state))
            #decode
            ...
            im = im.reshape(D, 1)
            print("{} {}".format(im_tilde.shape, x_tilde.shape))
            print("{}".format(np.linalg.norm(im - x_tilde)))
